...
@section('title', 'Edit Mesin - Sistem Laporan Maintenance')

@section('content')
<div class="card">
    <div class="card-header">
        <h4 class="mb-0">Edit Mesin</h4>
    </div>
    <div class="card-body">
        <form action="{{ route('machines.update', $machine->id) }}" method="POST">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('line_id') is-invalid @enderror" id="line_id" name="line_id" required>
                        ...
                        @foreach($lines as $line)
                            <option value="{{ $line->id }}" {{ old('line_id', $machine->line_id) == $line->id ? 'selected' : '' }}>
                                {{ $line->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                    id="description" name="description" rows="3">{{ old('description', $machine->description) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                    <option value="active" @selected(old('status', $machine->status) === 'active')>Aktif</option>
                    <option value="inactive" @selected(old('status', $machine->status) === 'inactive')>Tidak Aktif</option>
                </select>
