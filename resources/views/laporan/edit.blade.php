        ...
        <form action="{{ route('laporan.update', $laporan->id) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="hidden" id="line_id_hidden" name="line_id" value="{{ old('line_id', $laporan->machine?->line_id) }}">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="btn-group w-100" role="group">
                        <input type="radio" class="btn-check" name="line_status" id="line_on" value="on" 
                            @checked(old('line_status', $laporan->line_status ?? 'on') === 'on')>
                        ...
                        <input type="radio" class="btn-check" name="line_status" id="line_off" value="off" 
                            @checked(old('line_status', $laporan->line_status) === 'off')>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.fn.select2) {
            ...
            const spEl = document.getElementById('spare_part_id');
            if (spEl) spEl.addEventListener('change', displaySparePartStock);
        }
        ...
        // Load existing spare parts from laporan data
        loadExistingSpareParts();
    ...
    // Load existing spare parts from the laporan being edited
    function loadExistingSpareParts() {
        @if($laporan->spare_parts_used && count($laporan->spare_parts_used) > 0)
            const existingParts = @json($laporan->spare_parts_used);
            if (existingParts && existingParts.length > 0) {
                existingParts.forEach(function(part) {
                    sparePartsList.push({
                        ...
                        komentar: part.komentar || '',
                        availableStock: 999 // existing parts don't need stock validation
                    });
                });
                updateSparePartsDisplay();
                // Auto-show the spare parts form if there are existing items
                document.getElementById('sparePartsFormContainer').style.display = 'block';
            }
        @elseif($laporan->spare_part_id)
            // Fallback: load from legacy single spare part fields
            sparePartsList.push({
                id: '{{ $laporan->spare_part_id }}',
                name: '{{ $laporan->sparePart?->name ?? "Spare Part" }}',
                qty: parseInt('{{ $laporan->qty_sparepart ?? 1 }}'),
                komentar: '{{ $laporan->komentar_sparepart ?? "" }}',
                availableStock: 999
            });
            updateSparePartsDisplay();
            document.getElementById('sparePartsFormContainer').style.display = 'block';
